package main.java.org.cal.fonctionel;
...
public class MainFonctionnel {
    ...
    private static List<Student> studentsAgeGt25SortedDescByAge() {
        return students.stream().filter(student -> student.age > 25)
                .sorted(Comparator.comparingInt(Student::age).reversed()).toList();
    }

    private static List<Student> sortedByFirstNameThenByLastName() {
        return students.stream()
                .sorted(Comparator.comparing(Student::prenom).thenComparing(Student::nom)).toList();
    }
    ...
    private static void groupByGenre() {
        ...
        final Map<String, List<Student>> collect = students.stream()
                .collect(Collectors.groupingBy(Student::genre));
